...
class YNH_API {
	...
	public function __construct ($host, $admin_password, $verify_ssl = false) {
		if (empty($host) || empty($admin_password)) {
			throw new \Exception("Host/Admin Password required for YNH_API object constructor.", 1);
		}
		// Check hostname resolves.
		/*if (gethostbyname($host) == $host && !filter_var($host, FILTER_VALIDATE_IP)) {
			throw new \Exception("Cannot resolve {$host}.", 2);
		}*/
	...
	public function login () {
		...
		if (!$cookies) {
			error_log("Failed to log into YunoHost.". $login);
			return false;
		}
		else {
	...
	private function action ($action_path, $http_method, $put_post_parameters = null) {
		...
		if (!$this->login_cookie && $action_path != '/postinstall') {
			throw new \Exception("Not logged into YunoHost. No Login cookie found or cookie expired.", 3);
		}
		...
		if (substr($header_response, 0, 9) == "HTTP/1.1 ") {
			...
			if ($http_response_line[1] == "200" OR $http_response_line[1] == "204" OR $http_response_line[3] == "201") {
				...
				// Failed response
				throw new \Exception("API Request Failed: {$body_response}" .
					" / Request - {$header_response}");
				return $body_response; // Return error message
			}
		} else {
			throw new \Exception("Error - Invalid HTTP Response" . $header_response);
			return false;
		}
		...
		if (!empty($action_response_array)) {
			...
			throw new \Exception("\$action_response_array is empty. Returning false.\n" . 
